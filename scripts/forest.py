...
class PCSFInput(object):
    def __init__(
        ...
        """ Converts input information into dictionaries to be used in the
        message passing algorithm

        INPUT: prizeFile - tab-delimited text file containing all proteins with
                           prizes formatted like "ProteinName\tPrizeValue"
                           edgeFile - tab-delimited text file containing edges
                           in interactome and their weights formatted like
                           "ProteinA\tProteinB\tWeight\t Directionality(U or D,
                           optional)" confFile - text file containing values
                           for all parameters. Should include the lines
                           "w=<value>", "D=<value>", and "b=<value>".
                           dummyMode - a string that indicates which nodes in
                           the interactome to connect the dummy node
                           to. 'terminals'=connect to all terminals (default),
                           'others'=connect to all nodes except for terminals,
                           'all'=connect to all nodes in the interactome, or a
                           the path to a text file containing a list of
                           proteins to connect to.
        OUTPUT: self.origPrizes - dictionary of proteins with
                prizes. {ProteinName: PrizeValue} self.negPrizes - dictionary
                of proteins with negative prizes calculated based on
                degree. {ProteinName: PrizeValue} self.totalPrizes - dictionary
                of prizes as msgsteiner sees them (PrizeValue*beta)
                self.dirEdges - dictionary of dictionaries of all directed
                edges.  {ProteinFROM: {ProteinTO: weight}} self.undirEdges -
                dictionary of dictionaries of all undirected edges.  {ProteinA:
                {ProteinB: weight}, ProteinB: {ProteinA: weight}}
                self.dummyNodeNeighbors - a list of all proteins that the dummy
                node should have edges to.  self.interactomeNodes - a list of
                all nodes in the interactome self.w, self.b, self.D, self.gb,
                self.mu, self.g, self.r, self.threads, self.noise - parameters
        """
        if prizeFile is None or edgeFile is None:
            sys.exit("PCSF.py failed. Needs -p and -e arguments."
                     "Run PCSF.py -h for help.")
        warnings = 0
        selfedges = 0
        knockoutCount = 0
        ...
        if (
            dummyMode != "terminals"
            and dummyMode != "all"
            and dummyMode != "others"
        ):
            try:
                dummyFile = open(dummyMode, "r")
            except Exception:
                sys.exit(
                    'dummyMode value not recognized. Accepted values include'
                    ' "all", "terminals", "others", or a path to a text file'
                    ' on your computer containing a list of proteins.'
                )
        ...
        try:
            ...
            processes = None
        try:
            print("Continuing with parameters w = %f, b = %f, D = %i, mu = %f,"
                  " g = %f, garnetBeta = %f, r = %f, noise = %f."
                  % (float(w), float(b), int(D), mu, g, gb, r, noise))
        except Exception:
            sys.exit("ERROR: There was a problem reading the file containing"
                     " parameters. Please include appropriate values for w, b,"
                     " D, and optionally mu, r, garnetBeta, g, or noise.")
        self.w = float(w)
        self.b = float(b)
        self.D = int(D)
        self.mu = mu
        ...
        print("Reading text file containing interactome edges: %s..."
              % edgeFile)
        dirEdges = {}
        undirEdges = {}
        interactomeNodes = []
        ...
        if len(words) == 3:
            col = 3
            print("File does not contain direction information."
                  " Treating all edges as undirected\n")
        elif len(words) == 4:
            col = 4
            print("File contains four columns. Fourth column will be"
                  " interpreted as directionality information\n")
        else:
            print("current line:", line)
            sys.exit("ERROR: File containing interactome edges should have 3"
                     " or 4 columns: ProteinA\tProteinB\tWeight\t"
                     "Directionality(U or D). Protein names should not"
                     " have spaces.")
        # to avoid printing out too many warnings,
        # here is a way to tally up edited edges
        below_0, above1 = 0, 0
        # Add each edge in edgeFile to undirEdges or dirEdges dictionary,
        # appropriately
        while line:
            words = line.strip().split()
            if len(words) != col:
                print("current line:", line)
                sys.exit("ERROR: All lines in the file containing the"
                         " interactome edges should have the same number"
                         " of columns. Protein names should not have spaces.")
            # Check for knockouts:
            ...
            try:
                ...
                sys.exit("ERROR: Your interactome edge file include a"
                         " non-numerical value in the third column."
                         " Aborting program.")
            ...
            if words[0] == words[1]:
                ...
                line = e.readline()
                continue
            # Add all edges to undirEdges if there is no directionality
            # information
            if col == 3:
            ...
            if col == 4:
                if words[3] == "U":
                    # If there are directed and undirected edges for the same
                    # protein pair, only keep directed
                    ...
                    if words[0] not in undirEdges:
                        ...
                        if words[1] not in undirEdges[words[0]]:
                            ...
                            # If there are duplicate edges, keep the highest
                            # weight
                            if words[2] > undirEdges[words[0]][words[1]]:
                                undirEdges[words[0]][words[1]] = words[2]
                    # Undirected edges represented as two directed edges
                    ...
                    if words[0] not in dirEdges:
                        ...
                        if words[1] not in dirEdges[words[0]]:
                            ...
                            if words[2] > dirEdges[words[0]][words[1]]:
                                dirEdges[words[0]][words[1]] = words[2]
                    # If there are directed and undirected edges for the same
                    # protein pair, keep only directed
                    if words[0] in undirEdges:
                        if words[1] in undirEdges[words[0]]:
                            del undirEdges[words[0]][words[1]]
                            del undirEdges[words[1]][words[0]]
                            if len(list(undirEdges[words[0]].keys())) == 0:
                                del undirEdges[words[0]]
                            if len(list(undirEdges[words[1]].keys())) == 0:
                                del undirEdges[words[1]]
                else:
                    print(("current line:", line))
                    sys.exit("ERROR: The fourth column in the file containing"
                             " interactome edges should only contain U or D.")
        ...
        # dirEndpoints stores edge endpoints of all directed edges together in
        # one set dirEdges is dict<str, dict<str, str>>
        dirEndpoints = set()
        for k, v in list(dirEdges.items()):
            dirEndpoints.add(k)
        ...
        if above1 > 0:
            print("WARNING!! All edgeweights should be a probability of"
                  " protein interaction. "
                  + str(above1)
                  + " of your edge weights include a number greater than 0.99."
                  " These were changed to 0.99...\n")
        if below_0 > 0:
            print("WARNING!! All edgeweights should be a probability of"
                  " protein interaction. "
                  + str(below_0)
                  + " of your edge weights include a number below than 0. "
        ...
        while line:
            ...
            if len(words) != 2:
                print(("current line:", line))
                sys.exit("ERROR: File containing prizes should have exactly"
                         "two columns: ProteinName\tPrizeValue. Protein names"
        ...
        if garnet is not None:
            print("Reading text file containing TF regression results:"
                  "%s...\n" % garnet)
            if os.path.exists(garnet):
                with open(garnet, "r") as g:
                    line = g.readline()
                    while line:
                        words = line.strip().split()
                        if len(words) != 2:
                            print("current line:", line)
                            sys.exit("ERROR: File containing TFs should have"
                                     " exactly two columns: TF_Name\t"
                                     "PrizeValue. TF names should not have"
                                     " spaces.")
                        # Increase count if this is not in the interactome
                        if (
                            words[0] not in undirEdges
                            and words[0] not in dirEndpoints
                        ):
                            count += 1
                        else:
                            # Scale prize using garnetBeta
                            prize = float(words[1]) * self.gb
                            # If the TF already has a prize value this will
                            # replace it.
                            origPrizes[words[0]] = prize
                            if words[0] in list(terminalTypes.keys()):
        ...
        percentexcluded = (
            count / float(len(list(origPrizes.keys())) + count)
        ) * 100
        if percentexcluded > 90:
            sys.exit(
                "ERROR: %i percent of your prize nodes are not included in"
                " the interactome! Make sure the protein names you are using"
                " are the same in your prize file as in your edge file."
                " Aborting program.\n"
                % percentexcluded
            )
        elif percentexcluded > 0:
            print(
                "WARNING!! %.3f percent of your prize nodes are"
                " not included in the interactome! These nodes"
                " were ignored. Make sure the protein names you"
                " are using are the same in your prize file as in"
                " your edge file. Continuing program...\n"
                % percentexcluded
            )
            warnings += 1
        ...
        if selfedges > 0:
            print(
                    "WARNING: There were %i self-edges in your interactome."
                    " We ignored these edges.\n" % selfedges
            )
            warnings += 1
        ...
        if knockoutCount > 0:
            print(
                "There were %i edges connected to your knockout"
                " protein(s). We ignored these edges.\n" % knockoutCount
            )
        ...
        if dummyMode == "terminals":
            dummyNodeNeighbors = list(origPrizes.keys())
            print(
                "Dummy node has been added, with edges to all"
                " %i nodes which were assigned prizes.\n"
                % len(list(origPrizes.keys()))
            )
        elif dummyMode == "all":
            dummyNodeNeighbors = interactomeNodes
            print(
                "Dummy node has been added, with edges to all %i nodes"
                " in the interactome.\n"
            ...
            for node1 in undirEdges:
                for node2 in undirEdges[node1]:
                    if (
                        node2 not in origPrizes
                        and node2 not in nonterminalNodes
                    ):
            ...
            for node1 in dirEdges:
                for node2 in dirEdges[node1]:
                    if (
                        node2 not in nonterminalNodes
                        and node2 not in origPrizes
                    ):
                        nonterminalNodes.append(node2)
            ...
            print(
                "Dummy node has been added, with edges to all %i nodes in"
                " the interactome which have not been assigned prizes.\n"
                % len(nonterminalNodes)
            )
        else:
            # Keep track of how many genes on dummyNeighbors list are actually
            # in interactome
            countNeighbors = 0.00
            numExcluded = 0
            ...
            if countNeighbors == 0:
                sys.exit(
                    "The file you provided for dummyMode does not"
                    " contain any proteins in the interactome. Each"
                    " line in your text file should contain the name of"
                    " one protein. Make sure the names are the same as"
                    " in the interactome."
                )
            percentexcluded = (numExcluded / countNeighbors) * 100
            # Warning if too many proteins in the file are excluded from
            # dummyNodeNeighbors
            if percentexcluded > 0:
                print(
                    "WARNING!! %i percent of the proteins listed"
                    " in dummyNeighbors.txt are not included in"
                    " the interactome! Make sure the protein names"
                    " you are using are the same in this file as"
                    " in the interactome. Continuing program...\n"
                    % percentexcluded
                )
                warnings += 1
            print(
                "Dummy node has been added, with edges to all %i nodes in"
                " the interactome listed in your dummyMode file.\n"
                % int(countNeighbors)
            )
        ...
        if warnings > 0:
            print(
                "THERE WERE %s WARNING(S) WHEN READING THE INPUT FILES.\n"
                % warnings
            )
    ...
    def assignNegPrizes(self, musquared, excludeT):
        ...
        if self.mu != 0.0:
            print(
                "Adding negative prizes to nodes in interactome using mu"
                " parameter..."
            )
            if musquared:
                print("Negative prizes will be proportional to node degree^2.")
            if excludeT:
                print(
                    "Terminals will retain their assigned prizes,"
                    " no negative prizes."
                )
            ...
            for prot in self.origPrizes:
                if not excludeT:
                    try:
                        ...
                        prize = (
                            self.b * float(self.origPrizes[prot])
                        ) + negprize
    ...
    def getInputInfo(self):
        """
        Prints the input information that this input object contains."
        Mostly used in debugging.
        """
        print("The input prizes were", self.origPrizes)
        print(
            "All undirected edges in the input interactome were",
            self.undirEdges,
        )
        print(
            "All directed edges in the input interactome were", self.dirEdges
        )
        print(
            "The dummy node was connected to nodes: "
            + str(self.dummyNodeNeighbors)
        )
        print(
            "The parameters were: w= "
            + self.w
            + " b= "
            + self.b
            + " D= "
    ...
    def runPCSF(self, msgpath, seed):
        """
        Passes the information in this input object to msgsteiner, and
        returns the results.

        INPUT: msgpath - points to the directory where msgsteiner is
        held.

        RETURNS: edgeList - the contents of stdout from msgsteiner: a
                            list of edges in the optimal Forest info -
                            the contents of stderr from msgsteiner: if
                            all goes well, a report on the
                            optimization
        """
        print(
            "Preparing information to send to the message passing"
            " algorithm...\n"
        )
        # Create a list of the input information for the msgsteiner subprocess
        input = tempfile.TemporaryFile(mode="r+")
        for edgeNode1 in self.dirEdges:
            for edgeNode2 in self.dirEdges[edgeNode1]:
                # directed edges are flipped so that they point towards the
                # root node Weights are converted to costs by using 1-weight
                # (good for Psiquic, needs to be changed -log2(weight)
                # for String)
                input.write(
                    "D %s %s %f\n"
                    % (
        ...
        for edgeNode1 in self.undirEdges:
            for edgeNode2 in self.undirEdges[edgeNode1]:
                # keep track of undirected edges added so they are only
                # included once in inputList
                try:
        ...
        # Run msgsteiner as subprocess. Using temporary files for stdin and
        # stdout to avoid broken pipes when data is too big
        subprocArgs = [
            msgpath,
            "-d",
            str(self.D),
        ...
        # Use the seed to set the -s (instance seed, which controls random
        # noise on edge weights) and the -z (message seed, which affects the
        # message passing) msgsteiner seeds
        if seed is not None:
        ...
        print(
            "Message passing run finished with the parameters: w = %s,"
            " b = %s, D = %s, mu = %s, g = %s, r = %s\n"
...
class PCSFOutput(object):
    def __init__(
        self, inputObj, edgeList, info, outputpath, outputlabel, betweenness
    ):
        """ Takes the forest output given by msgsteiner and converts
        it to two networkx graphs.

        INPUT: inputObj - a reference to the PCSFInput object that
                          stores the correct edges and prizes
                          dictionaries for this output object edgeList
                          - the edges in the forest output given by
                          msgsteiner, contents of stdout info - stats
                          about the msgsteiner run, contents of stderr
                          outputpath - path to the directory where
                          output files should be stored outputlabel -
                          a label with which to name all of the output
                          files for this run betweenness - a T/F flag
                          indicating whether we should do the costly
                          betweenness calculation

        OUTPUT: self.optForest - a networkx digraph storing the forest
                returned by msgsteiner self.augForest - a networkx
                digraph storing the forest returned by msgsteiner,
                plus all of the edges in the interactome between nodes
                in that forest self.dumForest - a networkx digraph
                storing the dummy node edges in the optimal forest
                self.inputObj - a reference to the PCSFInput object
                that created this output object <outputlabel>_info.txt
                - a text file containing the contents of stderr and
                info """
        # Write output stderr file before attempting to do anything else so the
        # info is there if the program breaks
        err = open("%s/%s_info.txt" % (outputpath, outputlabel), "w")
        err.write(info)
        ...
        for edge in edges:
            ...
            if len(words) > 0:
                ...
                if words[0] == "DUMMY":
                    sys.exit(
                        "ERROR: Tree returned from message passing"
                        " algorithm has incorrect edges pointing towards"
                        " the root dummy node!"
                    )
                if words[1] == "DUMMY":
                ...
                try:
                    ...
                    try:
                        ...
                        if (
                            inputObj.undirEdges[words[1]][words[0]]
                            != inputObj.undirEdges[words[0]][words[1]]
                        ):
                            sys.exit(
                                "ERROR: Undirected edges must have symmetric"
                                " costs"
                            )
                    ...
                    # edge not found in either dictionary
                        ...
                        sys.exit(
                            "ERROR: Edges were returned from the message"
                            " passing algorithm that were not found in the"
                            "  input data. Aborting program."
                        )

        # Initially store all of the prizes, then subtract the prize of those
        # that are in the optimal forest
        prizeTerm = sum(
            inputObj.totalPrizes.values()
        )  # cummulative prizes of excluded nodes

        # Add prizes from input data as node attribute. Steiner nodes have
        # prize zero. Also add fracOptContaining as node attribute.
        terminalCount = 0
        ...
        err.write("\n")
        err.write(
            "There were %i terminals in the interactome.\n"
            % len(list(inputObj.origPrizes.keys()))
        )
        err.write(
            "There are %i terminals in the optimal forest.\n" % terminalCount
        )
        err.write(
            "The optimal forest has %i nodes total.\n" % len(optForest.nodes())
        )
        opt_roots = [node for node in dumForest.nodes() if node != "DUMMY"]
        err.write(
            "The %d roots in the optimal forest are %s\n"
            % (len(opt_roots), str(opt_roots))
        )
        singletons = [
            node
            for node in dumForest.nodes()
            if node != "DUMMY" and node not in optForest.nodes()
        ]
        err.write(
            "The %d singleton roots that will not show up in "
    ...
    def writeCytoFiles(self, outputpath, outputlabel, cyto30):
        ...
        INPUT: outputpath - path to the directory where output files should be
                            stored
               outputlabel - a label with which to name all of the output files
                             for this run

        OUTPUT: <outputlabel>_optimalForest.sif - a file storing edges of
                                                  optForest in Simple
                                                  Interaction Format
                <outputlabel>_augmentedForest.sif - a file storing edges of
                                                    augForest in Simple
                                                    Interaction Format
                <outputlabel>_dummyForest.sif - a file storing edges of
                                                dumForest in Simple
                                                Interaction Format
                <outputlabel>_nodeattributes.tsv - a tab-delimited file
                                                   storing the node attributes
                <outputlabel>_edgeattributes.tsv - a tab-delimited file storing
                                                   the edge attributes
        """
        if cyto30:
            # Write Simple Interaction Format files to store edges in a format
            # supported by Cytoscape 3.0
            optSif = open(
                "%s/%s_optimalForest.sif" % (outputpath, outputlabel), "w"
            )
            augSif = open(
                "%s/%s_augmentedForest.sif" % (outputpath, outputlabel), "w"
            )
            dumSif = open(
                "%s/%s_dummyForest.sif" % (outputpath, outputlabel), "w"
            )

            # Write attribute files in a format supported by Cytoscape
            # The first lines of these files contains the variable names
            noa = open(
                "%s/%s_nodeattributes.tsv" % (outputpath, outputlabel), "w"
            )
            noa.write(
                "Protein\tPrize\tBetweennessCentrality\t"
                "FractionOfOptimalForestsContaining\tTerminalType\n"
            )
            eda = open(
                "%s/%s_edgeattributes.tsv" % (outputpath, outputlabel), "w"
            )
            ...
            for (node1, node2, data) in edgesSorted:
                ...
                # Else undirected
                    ...
                    # Don't want to write undirected interactions twice
                    # (A pp B, B pp A).
                    try:
            ...
            eda.close()
            print(
                "Wrote output files for Cytoscape, in directory %s, with"
            ...
            # Write attribute files in a format supported by Cytoscape
            bcNoa = open(
                "%s/%s_betweennessCentrality.noa" % (outputpath, outputlabel),
                "w",
            )
            bcNoa.write("BetweennessCentrality (class=Double)\n")
            prizeNoa = open(
                "%s/%s_prizes.noa" % (outputpath, outputlabel), "w"
            )
            prizeNoa.write("Prize (class=Double)\n")
            fracNoa = open(
                "%s/%s_fracOptContaining.noa" % (outputpath, outputlabel), "w"
            )
            fracNoa.write("FractionOptimalForestsContaining (class=Double)\n")
            ttypeNoa = open(
                "%s/%s_termTypes.noa" % (outputpath, outputlabel), "w"
            )
            ttypeNoa.write("TerminalType\n")

            # Write edge attribute files in a format supported by Cytoscape
            weightEda = open(
                "%s/%s_weights.eda" % (outputpath, outputlabel), "w"
            )
            weightEda.write("Weight (class=Double)\n")
            fracEda = open(
                "%s/%s_fracOptContaining.eda" % (outputpath, outputlabel), "w"
            )
            ...
            for (node, data) in nodesSorted:
                ...
                fracNoa.write(
                    node + " = " + str(data["fracOptContaining"]) + "\n"
                )
            ...
            fracEda.close()
            print(
                "Wrote output files for Cytoscape, in directory %s, with names"
                " starting with "
                '"%s".\n' % (outputpath, outputlabel)
            )
...
def mergeOutputs(PCSFOutputObj1, PCSFOutputObj2, betweenness, n1=1, n2=1):
    """
    Merges two PCSFOutput objects together. Creates a new PCSFOutput
    object whose graphs contain all edges found in either original
    object, with updated fracOptContaining values and betweenness
    values.

    INPUT: Two PCSFOutput objects, either individual objects or
                themselves results of merges.  Ideally, these output
                objects were created using the same interactome
                (though the prizes or algorithm parameters may have
                been different). This is not enforced.  betweenness -
                a T/F flag indicating whether to do the costly
                betweenness calculation n1,n2 - integers, the number
    ...
    RETURNS: A new PCSFOutput object, with all edges found in either
                ...
                original objects, the prize or weight in this object
                is copied from PCSFOutputObj1. The inputObj reference
                in this object is the same as PCSFOutputObj1.  """
    print(
            "Merging outputs to give summary over %i algorithm runs..."
            % (n1 + n2)
    )
    ...
    for (node1, node2, data) in PCSFOutputObj1.optForest.edges(data=True):
        ...
        try:
            ...
            numRuns2 = (
                PCSFOutputObj2.optForest[node1][node2]["fracOptContaining"]
                * n2
            )
        except KeyError:
            numRuns2 = 0.0
        mergedObj.optForest[node1][node2]["fracOptContaining"] = (
            numRuns1 + numRuns2
        ) / (n1 + n2)
    ...
    for (node1, node2, data) in PCSFOutputObj2.optForest.edges(data=True):
        try:
            mergedObj.optForest[node1][node2]
        except KeyError:
            numRuns2 = data["fracOptContaining"] * n2
            # If there are nodes in outputObj2 not included in 1, they will be
            # added to mergedObj without error
            ...
            mergedObj.optForest.add_edge(
                node1,
                node2,
                weight=data["weight"],
                fracOptContaining=numRuns2 / (n1 + n2),
            )
    # Add dumForest edges to mergedObj that appear in outputObj2 but not in
    # outputObj1
    for (node1, node2, data) in PCSFOutputObj2.dumForest.edges(data=True):
        try:
            mergedObj.dumForest[node1][node2]
        except KeyError:
            mergedObj.dumForest.add_edge(node1, node2)

    # Create augForest based on new optForest
    # Need to first copy optForest in case an edge previously included in
    # augForest has a new fracOptContaining
    ...
    for node in mergedObj.augForest.nodes():
        ...
        try:
            ...
            # If a node found in mergedObj.optForest is not found in
            # PCSFInputObj1's interactome, it is quietly ignored in making
            # augForest
            pass
        for node2 in edges:
            if node2 in mergedObj.augForest.nodes():
                if (node, node2) not in mergedObj.optForest.edges():
                    mergedObj.augForest.add_edge(
                        node, node2, weight=edges[node2], fracOptContaining=0.0
                    )
...
def shufflePrizes(PCSFInputObj, seed, excludeT):
    ...
    INPUT: a PCSFInput object seed - number to give to the random
           number generator RETURNS: a new PCSFInput object with the
           same prize values, but in randomly shuffled order.  All
           nodes will now have random prizes, but with the same
    ...
    random.seed(seed)
    newNodes = random.sample(
        list(PCSFInputObj.totalPrizes.keys()), numTruePrizes
    )
    shuffledValues = dict(
        list(zip(newNodes, list(PCSFInputObj.origPrizes.values())))
    )
    # Make a new PCSFInput object that contains all the same values as
    # the original
...
def noiseEdges(PCSFInputObj, seed, excludeT):
    """
    Adds gaussian noise to all edges in the PCSFInputObj prize
    dictionary.

    INPUT: a PCSFInput object seed - number to give to the random
           number generator RETURNS: a new PCSFInput object with with
           added gaussian noise to edge values
    """
    # Make a new PCSFInput object that contains all the same values as the
    # original
    newPCSFInputObj = copy.deepcopy(PCSFInputObj)
    # Generate gaussian noise values, mean=0, stdev default=0.333 (edge
    # values range between 0 and 1)
    random.seed(seed)
    dev = PCSFInputObj.noise
    for node1 in newPCSFInputObj.dirEdges:
        for node2 in newPCSFInputObj.dirEdges[node1]:
            newPCSFInputObj.dirEdges[node1][node2] = float(
                PCSFInputObj.dirEdges[node1][node2]
            ) + random.gauss(0, dev)
    for node1 in newPCSFInputObj.undirEdges:
        for node2 in newPCSFInputObj.undirEdges[node1]:
            newPCSFInputObj.undirEdges[node1][node2] = float(
                PCSFInputObj.undirEdges[node1][node2]
            ) + random.gauss(0, dev)
...
def randomTerminals(PCSFInputObj, seed, excludeT):
    """
    Selects nodes with a similar degree distribution to the original
    terminals, and assigns the prizes to them.
    ...
    # Make a new PCSFInput object that contains all the same values as the
    # original but empty prizes
    newPCSFInputObj = copy.deepcopy(PCSFInputObj)
    newPCSFInputObj.origPrizes = {"": 0}
    # degrees is a sorted list that will hold outdegree of every node in
    # interactome
    degrees = []
        ...
        for node in PCSFInputObj.undirEdges:
            try:
                degrees.append(
                    (
                        node,
                        len(PCSFInputObj.undirEdges[node])
    ...
    for k, terminal in enumerate(PCSFInputObj.origPrizes):
        for i, value in enumerate(degrees):
            if terminal == value[0]:
                index = i
                break
        # Choose an index offset to select new terminal (distance from orig
        # terminal in degrees list) Make sure newly chosen terminal is not
        # already chosen on a previous round
        newTerm = ""
        i = -1
        while newTerm in newPCSFInputObj.origPrizes and i <= 10000:
            ...
            # if offset causes the index to wraparound to the other side of
            # the list, try again
            if newIndex < 0:
                continue
            try:
                ...
                continue
            # To make truly random, need to choose randomly between all nodes
            # with the same degree Otherwise, ordering of dict iteration
            # matters
            nodesWithSameDegree = []
...
def changeValuesAndMergeResults(
    ...
    """
    Changes the prizes/edges in the PCSFInput object according to func
    and runs the msgsteiner algorithm, then merges the results
    together with the given PCSFOutput object. Writes cytoscape files
    for final merged results.

    INPUT: func - the function which takes inputObj and changes the
                  prize/edge values (i.e. shuffles or adds noise)
                  inputObj - a PCSFInput object with original values,
                  to be changed.  numRums - the number of times to
                  change the values and re-run msgsteiner msgpath -
                  path to the directory where msgsteiner is kept
                  outputpath - path to the directory where output
                  files should be stored outputlabel - a label with
                  which to name all of the output files for this run

    OUTPUT: <outputlabel>_changed_#_info.txt - a text file FOR EACH
                      RUN containing the contents of stderr for all
                      msgsteiner runs RETURNS: merged - the PCSFOutput
                      object that is a result of all the merges
...
def crossValidation(k, rep, PCSFInputObj, seed, msgpath, outputpath,
                    outputlabel):
    """
    Seperates prizes into k "folds" and leaves those out of
    analysis. Reports what fraction of held-out prize nodes were
    returned as steiner nodes.

    INPUT: k - the number of "folds" to seperate the prize nodes into.
           rep - Repetition of k-fold cv calculation we are currently
           on PCSFInputObj - a PCSF object with all prize nodes seed -
           number to give to the random number generator msgpath -
           path to msgsteiner code outputpath - path to the directory
           where output files should be stored outputlabel - a label
           with which to name all of the output files for this run

    OUTPUTS: File <outputlabel>_cvResults_<rep>.txt containing stats
             from the cv run Files showing steiners and terminals for
             each of the intermediate solutions """
    print(("Running %i-fold cross validation (rep %i).\n" % (k, rep)))
    prizes = list(PCSFInputObj.origPrizes.keys())
    if seed is not None:
    ...
    for i in range(0, k):
        ...
        outputs = open(
            "%s/%s_cvIntermediate_rep%ik%i.txt"
            % (outputpath, outputlabel, rep, i),
            "w",
        )
        ...
        newPCSFInputObj = copy.deepcopy(PCSFInputObj)
        for p in hold_out:
            # Remove held out original prize and update total prize to reflect
            # only negPrize
        ...
        outputs.close
        # Return num of held-out terminals, num of recovered hold-outs, total
        # num of Steiner nodes
    ...
    results = open(
        "%s/%s_cvResults_%i.txt" % (outputpath, outputlabel, rep), "w"
    )
    results.write(
        "Iteration\tNum of held-out terminals\tNum of recovered terminals\t"
...
def main():
    ...
    parser.add_argument(
        "--msgpath",
        dest="msgpath",
        help="Full path to the message passing code. "
        'Default = "<current directory>/msgsteiner"',
        default="./msgsteiner",
    )
    parser.add_argument(
        "--outpath",
    ...
    # files
    try:
        with open(options.msgpath):
            pass
    except IOError:
        sys.exit(
            "ERROR: The msgsteiner code was not found in the correct"
            " directory. Please use --msgpath to specify the path to"
            " the msgsteiner code."
        )
